...
import CombinationItems from '@/components/questions/CombinationItems.vue';
...
export default class ShowCombinationQuestion extends Vue {
  @Prop({ type: CombinationQuestionDetails, required: true })
  readonly questionDetails!: CombinationQuestionDetails;
  ...
  getOptions(name: number, options: CombinationItem[]): Option[] {
    ...
    document.querySelectorAll('.cm-custom-drop-down').forEach((e, index) => {
      ...
      this.addOptions(
        ...
        this.getOptions(
          ...
          this.questionDetails.combinationItems
        )
      );
      d.selectedIndex = 0;
    });
