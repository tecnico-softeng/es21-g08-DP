describe('Manage Multiple Choice Questions Walk-through', () => {
  function validateQuestion(
    ...
    correctIndex = [2],
    relevance = 0,
    relevanceValues = [null,'Relevance: 2',null,'Relevance: 5',null,null,'Relevance: 3',null,'Relevance: 1',null]
  ) {
      ...
      .within(($ls) => {
        ...
        cy.get('li').each(($el, index, $list) => {
          ...
          if(correctIndex.includes(index)) {
            if(relevance == 0)cy.get($el).should('contain', '[★]');
            if(relevance == 1)cy.get($el).should('contain', relevanceValues[index]);
          } else{
  ...
  it('Creates a new multiple choice question with 10 options and 4 options true and with order of relevance', function() {
    cy.get('button')
    ...
    relevances = [null,'2{enter}',null,'5{enter}',null,null,'3{enter}',null,'1{enter}',null]
      ...
      .each(($el, index, $list) => {
        cy.get($el).within($ls => {
          if (index === 1 || index=== 3 || index=== 6 || index === 8) {
            cy.get(`[data-cy="Switch${index + 1}"]`).check({ force: true });
            cy.get(`[data-cy="Relevance${index + 1}"]`).type(relevances[index], {force:true})
          }
