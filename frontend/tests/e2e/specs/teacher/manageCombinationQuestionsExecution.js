describe('Manage Item Combination Questions Walk-through', () => {
    ...
    before(() => {
      cy.cleanCombinationQuestionsByName('Cypress Question Example');
    ...
    it('Creates a new item combination question', function() {
      ...
      cy.get('button')
        .contains('answer item group 1', { matchCase: false })
        .click({ force: true });
    ...
    it('Can update title (with right-click) on item combination question', function() {
      ...
      cy.get('[data-cy="createOrEditQuestionDialog"]')
        ...
        .within($list => {
          ...
          cy.get('[data-cy="questionTitleTextArea"]')
            .clear({ force: true })
            .type('Cypress Item Combination Question Example - 01 - Edited', { force: true });
      ...
      cy.get('[data-cy="questionTitleGrid"]')
        .first()
        .should('contain', 'Cypress Item Combination Question Example - 01 - Edited');
  
      validateQuestionFull(
        (title = 'Cypress Item Combination Question Example - 01 - Edited'),
        (content = 'Cypress Question Example - Content - 01')
      );
    });
  
    it('Can update content (with button) on item combination question', function() {
      cy.route('PUT', '/questions/*').as('updateQuestion');
      ...
      cy.get('[data-cy="createOrEditQuestionDialog"]')
        ...
        .within($list => {
          ...
          cy.get('[data-cy="questionQuestionTextArea"]')
            .clear({ force: true })
            .type('Cypress New Content For Item Combination Question!', { force: true });
      ...
      validateQuestionFull(
        ...
        (content = 'Cypress New Content For Item Combination Question!')
      );
    });
    ...
    it('Can duplicate item combination question', function() {
        ...
        'have.value',
        'Cypress New Content For Item Combination Question!'
        );
