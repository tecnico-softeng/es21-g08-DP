...
public class QuestionsXmlImport {
    ...
    private void importQuestion(Element questionElement, Course course) {
        ...
        switch (type) {
            ...
            case Question.QuestionTypes.ITEM_COMBINATION_QUESTION:
                questionDetailsDto = importCombinationQuestion(questionElement);
                break;
    ...
    private QuestionDetailsDto importCombinationQuestion(Element questionElement) {
        CombinationQuestionDto questionDto = new CombinationQuestionDto();
        questionDto.setLanguage(Languages.valueOf(questionElement.getChild("code").getAttributeValue("language")));
        var groups = new ArrayList<CombinationGroupDto>();
        for (Element spotElement : questionElement.getChild("combinationGroup").getChildren("combinationOption")) {
            var spot = new CombinationGroupDto();
            spot.setSequence(Integer.valueOf(spotElement.getAttributeValue("sequence")));
            var options = new ArrayList<OptionDto>();
            for (Element optionElement : spotElement.getChildren("combinationOption")) {
                Integer optionSequence = Integer.valueOf(optionElement.getAttributeValue("sequence"));
                String optionContent = optionElement.getAttributeValue("content");
            ...
            spot.setItems(options);
            groups.add(spot);
        }
        questionDto.setCombinationGroup(groups);
        return questionDto;
    }
    ...
    private QuestionDetailsDto importOpenEndedQuestion(Element questionElement) {
        ...
        questionDto.setDefaultCorrectAnswer(questionElement.getChildText("defaultCorrectAnswer"));
