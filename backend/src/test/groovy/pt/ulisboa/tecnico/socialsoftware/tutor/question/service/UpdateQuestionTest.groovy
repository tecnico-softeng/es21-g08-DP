...
class UpdateQuestionTest extends SpockTest {
   ...
   def "update a question"() {
    ...
    def "update question with two options true and with order of relevance"() {
        ...
        resOptionOne.getContent() == OPTION_1_CONTENT
        resOptionOne.isCorrect()
        //resOptionOne.getRelevance() == 2 CONDIÇÃO FALHA AQUI
        def resOptionTwo = result.getQuestionDetails().getOptions().stream().filter({ option -> option.getId() == optionKO.getId()}).findAny().orElse(null)
        resOptionTwo.getContent() == OPTION_2_CONTENT
        resOptionTwo.isCorrect()
        //resOptionTwo.getRelevance() == 3  CONDIÇÃO FALHA AQUI
    ...
    def "update a combination question"() {
        given: "a changed question"
        def questionDto = new QuestionDto(questionC)
        questionDto.setTitle(QUESTION_2_TITLE)
        questionDto.setContent(QUESTION_2_CONTENT)
        questionDto.setQuestionDetailsDto(new CombinationQuestionDto())
        def groupDto = new CombinationGroupDto()
        groupDto.setTitle(QUESTION_1_TITLE)
        groupDto.setSequence(2)

        and: '2 changed items'
        def optionDto = new OptionDto(option1)
        optionDto.setContent(OPTION_2_CONTENT)
        optionDto.setCombination(LIST_1_COMBINATION)
        groupDto.getItems().add(optionDto)
        optionDto = new OptionDto(option2)
        optionDto.setCombination(LIST_1_COMBINATION)
        groupDto.getItems().add(optionDto)
        
        questionDto.setQuestionDetailsDto.getCombinationGroup().add(groupDto)
        ...
        and: 'an option is changed'
        result.getQuestionDetails().getOptions().size() == 2
        def resOptionOne = result.getQuestionDetails().getItems().stream().filter({ item -> item.getId() == optionOK.getId()}).findAny().orElse(null)
        resOptionOne.getContent() == OPTION_2_CONTENT
        def resOptionTwo = result.getQuestionDetails().getItems().stream().filter({ item -> item.getId() == optionOK.getId()}).findAny().orElse(null)
        resOptionTwo.getContent() == OPTION_1_CONTENT
    }
